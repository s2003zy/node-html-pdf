...
function PDF (html, options) {
  ...
  if (this.options.script) {
    ...
    this.script = path.join(getUpackAsraPath(__dirname), 'scripts', 'pdf_a4_portrait.js')
  }
...
PDF.prototype.exec = function PdfExec (callback) {
  var child = childprocess.spawn(getUpackAsraPath(this.options.phantomPath), [].concat(this.options.phantomArgs, [this.script]), this.options.childProcessOptions)
  var stderr = []
